...
def setVolume(serial_input, audioControlDict):
    for i in range(0,len(audioControlDict)):
        for u in range(0,len(audioControlDict[i])):
            if audioControlDict[i][u] == "master":
                masterMultiplier = serial_input[i]
    for i in range(0,len(audioControlDict)):
        for u in range(0,len(audioControlDict[i])):
            if audioControlDict[i][u] == "master":
                masterMultiplier = serial_input[i]
            try:
                audioControlDict[i][u].SetMasterVolume(float(serial_input[i])/100*(float(masterMultiplier)/100), None)
            except:
                continue
